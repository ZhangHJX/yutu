...
abstract class CanvasElement {
  final String id;
  Offset position;        // 逻辑坐标（左上角）
  double width;
  double height;
  double rotation;        // 角度（degree）
  double scale;           // 单个元素缩放（非画布）
  Matrix4 transform;      // 独立变换矩阵（命中检测）
  ...
  bool hitTest(Offset p) {
    Matrix4 inv = Matrix4.inverted(transform.clone());
    final v = inv.transform3(Vector3(p.dx, p.dy, 0));
    final local = Offset(v.x, v.y);
    return Rect.fromLTWH(0, 0, width, height).contains(local);
  }
}
