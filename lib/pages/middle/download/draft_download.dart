...
class DraftDownload extends BaseResourceDownload {
  ...
  Future<(bool exists, bool timestampMatches)> checkDraftResourceExists(
    ...
    try {
      ...
      if (draftModel == null) {
        AppLogger.info('DraftResourceDownloadService: 草稿 $id 在数据库中不存在');
        return (false, false);
      }
      ...
      if (exists) {
        AppLogger.info('DraftResourceDownloadService: 草稿 $id 存在且时间戳匹配');
        return (true, true);
      } else {
        AppLogger.info('DraftResourceDownloadService: 草稿 $id 数据库存在但文件不存在');
        return (true, false);
      }
    } catch (e) {
      AppLogger.error('DraftResourceDownloadService: 检查草稿资源失败:', e);
      return (false, false);
    }
  }
  ...
  Future<String> downloadDraftResource(
    ...
    if (exists && timestampMatches) {
      // 时间戳匹配，直接使用本地文件
      AppLogger.info('DraftResourceDownloadService: 草稿 $id 时间戳匹配，使用本地文件');
      onProgress?.call(1.0);
    ...
    AppLogger.info('DraftResourceDownloadService: 开始下载草稿资源文件 $id');
    ...
    try {
      ...
      AppLogger.info('DraftResourceDownloadService: 草稿资源文件 $id 下载完成');

      return await getDraftResourceFilePath(id);
    } catch (e) {
      AppLogger.error('DraftResourceDownloadService: 草稿资源文件下载失败:', e);
      rethrow;
    } finally {
      // 清理临时文件
      try {
        ...
        AppLogger.error('DraftResourceDownloadService: 清理临时文件失败: ', e);
      }
    }
  }
  ...
  Future<void> extractDraftZip(
    ...
    try {
      ...
      onProgress?.call(1.0);
      AppLogger.info(
        'DraftResourceDownloadService: 草稿资源文件解压完成: ${targetDir.path}',
      );
    } catch (e) {
      AppLogger.error('DraftResourceDownloadService: 解压草稿资源文件失败: ', e);
      rethrow;
    }
  }
  ...
  Future<bool> saveOrUpdateDraft(MiddleModel model) async {
    try {
      ...
      if (!success) {
        AppLogger.info('DraftResourceDownload: 保存数据库记录失败, id=${model.id}');
        return false;
      }
      AppLogger.info('DraftResourceDownload: 草稿更新或保存成功, id=${model.id}');
      return true;
    } catch (e, stackTrace) {
      AppLogger.error('DraftResourceDownload: 保存草稿失败: ', e, stackTrace);
      return false;
    }
  }
}
